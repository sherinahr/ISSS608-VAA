...
The summary function is used to provide an overview of the datasets.
...
The other change is to make the field "haveKids" clearer. Currently, it reflects the boolean values of True/False. To ensure clarity and better understanding of our subsequent charts, the values will be changed to Yes/No, as reflected in the below code chunk. As above, the mutate() function will be used to create a new variable called Have_Kids.

```{r}
ppt<-ppt %>%
  mutate(Have_Kids = case_when(haveKids==TRUE ~ "Yes", haveKids==FALSE ~ "No"))
...
Firstly, there are some duplicate rows in the dataset which have to be removed using the distinct() function. As the code below shows, there were 1,113 duplicate rows in the FinancialJournal dataset.
...
Next, there were some participants who did not have entries for all 6 months. To find the these participants, first the data was grouped by the Participant_ID. Examination of the results show that there are 131 participants which have lower than 1,000 transactions. 
...
The code chunk below will filter out these 131 participants and tell us how many rows were removed. The data frame fj_final now has no duplicates, and only residents who still live in City of Engagement.
...
Next, we would want to convert all of the timestamp data into Month-Year format for ease of comparison using the code chunk below. 
...
fj_final_by_year <- fj_final %>%
  group_by(participantId, category,Year_month) %>%
  summarise(Total = sum(amount))

fj_final_by_year <- pivot_wider(
  fj_final_by_year,names_from = category,values_from =Total)

# Replace NA values with 0 for Rent Adjustment
fj_final_by_year[is.na(fj_final_by_year)] <- 0
...
fj_final_by_year$Cost_of_living <- fj_final_by_year$Education + 
  fj_final_by_year$Food + fj_final_by_year$Recreation + 
  fj_final_by_year$Shelter

fj_final_by_year$Total_Income <- fj_final_by_year$Wage + 
  fj_final_by_year$RentAdjustment

fj_final_by_year$Total_Savings <- fj_final_by_year$Total_Income - fj_final_by_year$Cost_of_living

head(fj_final_by_year)
